...
If any changed files are under `portage-stubs/` or `tests/`, run the following
...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `portage-stubs/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `portage-stubs/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `portage-stubs/commands/main.py`,
...
For Python files, strip the `portage-stubs/` prefix and replace `/` with `.` (like module imports).

- Python file `portage-stubs/media.py` → `media:`.
- Multiple files under `portage-stubs/commands/` → `commands:`.
- Single command file `portage-stubs/commands/admin.py` → `commands.admin:`.
