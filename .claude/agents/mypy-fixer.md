...
Fixes mypy type errors and eliminates `Any` usage in the portage-stubs project.
...
1. Fix all mypy errors reported by `uv run mypy portage-stubs`
...
Place new TypedDict classes in `portage-stubs/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`portage-stubs/typing.py` when shared.
...
1. Run `uv run mypy portage-stubs` and capture all errors.
...
4. Run `uv run mypy portage-stubs` again to verify fixes.
